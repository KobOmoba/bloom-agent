...
function looksLikeName(raw) {
  ...
  s = s.replace(/^(Hon\/Snr\/|Snr\/Ldr\.|Sp\/Ven\/|Sp\/Sup\/|L\/S\/S\/E\/S\.|L\/S\/E\/S\.|C\/E\/B\.|M\/C\s|MC\.|C\/P\.|S\/P\/S\.|Evang\.|Rev\.|Pastor\s|Dr\.|Mr\.|Mrs\.|Miss\s)/gi, '').trim();
  if (!s || s.length < 3) return { ok: false, name: '' };
  const letters = s.replace(/[^a-zA-Z\s]/g, '').trim();
  if (letters.length < 3) return { ok: false, name: '' };
  const digitRatio = s.replace(/[^0-9]/g,'').length / s.length;
  if (digitRatio > 0.3) return { ok: false, name: '' };
  if (/^(general|members|list|students|class|section|total|name|s\/n|serial|no\.?|page|date|school)/i.test(letters.trim())) return { ok: false, name: '' };
  const letterRatio = letters.length / Math.max(s.length, 1);
  if (letterRatio < 0.5) return { ok: false, name: '' };
...
function readTextOrCSV(file) {
  ...
  reader.onload = ev => {
    ...
    const names = [];
    lines.forEach(line => {
      const col0 = line.trim().split(',')[0].replace(/"/g,'').trim();
      const { ok, name } = looksLikeName(col0);
