...
// Photos: AI OCR — AariNAT OCR (primary) → GLM-OCR (fallback)
...
function handleRegisterCSV(e) {
  ...
  if (ocrFiles.length) {
    // Always scan immediately — AariNAT OCR (primary), GLM-OCR (fallback).
...
// ═══════════════════════════════════════════════════════════════════════════
// OCR ENGINE — AariNAT OCR (primary) → GLM-OCR (fallback)
// Base44, Gemini, OCR.space removed
// ═══════════════════════════════════════════════════════════════════════════
...
// ── GLM-OCR — ZhipuAI GLM-4V-Flash vision model (fallback) ────────────────
// Free tier: https://open.bigmodel.cn — agents get key from Settings
const GLM_KEY_STORAGE = 'glm_api_key';
function getGLMKey() { return window.GLM_API_KEY || localStorage.getItem(GLM_KEY_STORAGE) || ''; }
const GLM_OCR_MODELS = ['glm-4v-flash', 'glm-4v-plus'];

const GLM_OCR_PROMPT = `You are reading a Nigerian primary/secondary school fee register.
...
async function glmOCR(base64, mime) {
  const apiKey = getGLMKey();
  if (!apiKey) throw new Error('No GLM API key');
  let lastError = null;
  for (const model of GLM_OCR_MODELS) {
    try {
      ...
      if (data.error) {
        lastError = data.error.message || ('GLM error ' + (data.error.code || ''));
        if (data.error.code === 401 || (lastError + '').toLowerCase().includes('auth'))
          throw new Error('GLM API key invalid — check in Settings');
        continue;
      }
      ...
      if (!Array.isArray(students) || !students.length) { lastError = model + ' returned 0 students'; continue; }
      ...
      console.log('✅ GLM-OCR (' + model + '): ' + normalized.length + ' names');
      return normalized;
    } catch (e) {
      lastError = e.message;
      console.warn('GLM ' + model + ' failed:', e.message);
      if (e instanceof SyntaxError) break;
    }
  }
  throw new Error('GLM-OCR failed: ' + lastError);
}
  ...
  return new Promise(resolve => {
    ...
    reader.onload = async ev => {
      ...
      try {
        ...
        console.warn('AariNAT OCR failed (' + reason + '), falling back to GLM-OCR...');
      }

      // ── 2. GLM-OCR — ZhipuAI GLM-4V-Flash (fallback) ────────────────
      //    Requires API key from open.bigmodel.cn — set in Settings
      try {
        ocrOverlayStep('upload', 'GLM-OCR scanning...', 50);
        const names = await glmOCR(b64, mime);
        if (names && names.length) {
          ocrOverlayStep('done', '✅ ' + names.length + ' names found — GLM-OCR', 100);
          resolve(names); return;
        }
      } catch (e) {
        console.warn('Page ' + pageNum + ' GLM-OCR failed:', e.message);
        // If no GLM key, give a helpful message
        if (e.message.includes('No GLM API key')) {
          ocrOverlayStep('error', '⚠️ GLM-OCR needs an API key — open Settings to add it (free at open.bigmodel.cn)', 100);
          resolve([]); return;
        }
      }
...
function renderSettingsProfile() {
  ...
  const maskedKey = glmKey ? glmKey.slice(0, 6) + '••••••' + glmKey.slice(-4) : '';
  ...
  c.innerHTML = `
    <div style="padding:1.2rem;">
      ...
      <div style="background:var(--card);border-radius:16px;padding:1.2rem;margin-bottom:1rem;">
        <div style="font-size:0.75rem;color:var(--sub);text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.6rem;">GLM-OCR API Key (Fallback Scanner)</div>
        <div style="font-size:0.78rem;color:var(--sub);margin-bottom:0.8rem;">
          AariNAT AI is the primary scanner. If it's unavailable, GLM-OCR takes over.<br>
          Get a free key at <a href="https://open.bigmodel.cn" target="_blank" style="color:#818cf8;">open.bigmodel.cn</a> → Console → API Keys → Create
        </div>
        <div style="font-size:0.82rem;color:#94a3b8;margin-bottom:0.5rem;">Current: ${maskedKey || '<span style="color:#f87171;">Not set</span>'}</div>
        <input id="glm-key-input" type="text" placeholder="Paste your GLM API key here..."
          ...
          value="">
        <button onclick="saveGLMKey()" style="margin-top:0.6rem;width:100%;padding:0.7rem;border-radius:10px;border:none;background:linear-gradient(135deg,#6366f1,#818cf8);color:white;font-weight:700;font-size:0.85rem;cursor:pointer;">
          💾 Save GLM Key
        </button>
      </div>
      ...
      <div style="text-align:center;font-size:0.7rem;color:var(--sub);padding:1rem 0;">
        ...
        v2.1 · OCR: AariNAT AI + GLM-4V-Flash
      </div>
    </div>
  `;
}
...
function saveGLMKey() {
  const input = document.getElementById('glm-key-input');
  if (!input) return;
  const key = input.value.trim();
  if (!key) {
    localStorage.removeItem(GLM_KEY_STORAGE);
    delete window.GLM_API_KEY;
    alert('GLM API key removed. AariNAT AI will still work as your primary scanner.');
    renderSettingsProfile();
    return;
  }
  localStorage.setItem(GLM_KEY_STORAGE, key);
  window.GLM_API_KEY = key;
  alert('✅ GLM API key saved! It will be used as fallback when AariNAT AI is unavailable.');
  renderSettingsProfile();
}
